...
// 获取按投稿时间倒序的视频列表，单次最多MaxFeedVideosNumOnce个
func Feed(c *gin.Context) {
	latestTimeStr := c.Query("latest_time")
	var latestTime int64
	// latest_time不填表示当前时间
	if latestTimeStr != "" {
		lTime, err := strconv.ParseInt(latestTimeStr, 10, 64)
		if err != nil {
			c.JSON(http.StatusOK, global.Response{StatusCode: 1, StatusMsg: "Get latestTime error"})
			return
		}
	...
	videoList, nextTime, errMsg := service.GetFeedVideoList(latestTime, token)
	// 如果服务出错，返回服务器错误响应
	if errMsg != "" {
